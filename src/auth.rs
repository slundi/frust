...
pub(crate) async fn check_token(pool: &actix_web::web::Data<crate::db::db::Pool>, req: actix_web::HttpRequest) -> Option<crate::model::Account> {
    ...
    if let Some(token_h) = value {
        ...
        if let Some(token) = token_cleaned {
            ...
            let result = std::future::IntoFuture::into_future(crate::db::db::get_user_from_token(&conn, token.to_owned())).await;
